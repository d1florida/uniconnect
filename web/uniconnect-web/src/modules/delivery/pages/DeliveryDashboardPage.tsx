...
export function DeliveryDashboardPage() {
  ...
  const heldCount = dash?.ordersHeldForFixedRoutes ?? 0;
  const dailyReady = dash?.ordersReadyToPlan ?? dash?.openOrders ?? 0;
  ...
  return (
    <div>
        ...
        <section className="today-strip">
          ...
          <div className="today-strip-items">
            <Link to={`/delivery/fleets/${tenantId}/orders`} className="today-strip-item">
              <span className="value">{dailyReady}</span>
              <span className="label">daily ready to plan</span>
            </Link>
            {heldCount > 0 && (
              <Link to={`/delivery/fleets/${tenantId}/orders?filter=held`} className="today-strip-item">
                <span className="value">{heldCount}</span>
                <span className="label">held for fixed routes</span>
              </Link>
            )}
      ...
      {dash && (
        <div className="cards">
          <div className="card">
            <div className="value">{dailyReady}</div>
            <div className="label">Daily ready</div>
          </div>
          <div className="card">
            <div className="value">{heldCount}</div>
            <div className="label">Held for fixed routes</div>
          </div>
      ...
      {tenantId && (
        <p>
          ...
          {' · '}
          <Link to={`/delivery/fleets/${tenantId}/driver-calendar`}>Calendar</Link>
          {' · '}
